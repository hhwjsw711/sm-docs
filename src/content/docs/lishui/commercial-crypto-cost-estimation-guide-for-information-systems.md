...
- AFP — 调整后的功能点数，单位为功能点；
- UFPI — 未调整的功能点数，单位为功能点；
- RUFI — 吻合度调整因子，取值见附录C中表C.1；
- MCAi — 修改类型调整因子，取值见附录C中表C.2；
- CF — 规模调整因子，取值见附录C中表C.3。
...
- SWF — 软件因素调整因子；
- SWF1 — 应用类型调整因子，取值见附录D中表D.1；
- C1 — 质量特性调整因子之分布式处理因子，取值见附录D中表D.2；
- C2 — 质量特性调整因子之性能因子，取值见附录D中表D.2；
- C3 — 质量特性调整因子之可靠性因子，取值见附录D中表D.2；
- C4 — 质量特性调整因子之多重站点因子，取值见附录D中表D.2。
...
- RDF — 软件因素调整因子；
- RDF1 — 开发语言调整因子，取值见附录D中表D.3；
- RDF2 — 开发团队背景调整因子，取值见附录D中表D.4。
...
d） 该 EQ 不应被重复计数。任何被分别计数的两个 EQ 至少满足下面一个条件（否则被视为同一 EQ）：
...
- （3）需要单独采购ssl密码模块1个、国密浏览器3个、国密站点证书1个。
...
|  RDF-开发因素调整因子 |  | 1 |  |   |
|  RDF1-开发语言调整因子 | 使用 java 语言开发 | 1 |  | 参考表 D. 3  |
